...
POLICY = load_policy(ROOT / "policies", "ops-agent")
...
def make(tmp_path: Path, script: list[Resp], rule: Callable[[TraceEvent], str]):
    ...
    harness = AgentHarness(client, hooks, store, model="claude-opus-5", use_fallbacks=False)
...
def external_email(e: TraceEvent) -> str:
    if e.kind == "tool_call" and e.tool_name == "send_email":
        return "violation" if any(not a.endswith("@acme.com") for a in e.arguments["to"]) else "allow"
    return "allow"
...
EMAIL_SCRIPT = [
    Resp(
        [
            Block("text", text="Sending now."),
            Block("tool_use", id="tu_1", name="send_email", input={"to": ["x@gmail.com"], "subject": "s", "body": "b"}),
        ],
        "tool_use",
    ),
    Resp([Block("text", text="I couldn't send that.")], "end_turn"),
]
...
def test_enforce_blocks_tool_call_and_agent_is_told(tmp_path):
    harness, store, client, _, sid = make(tmp_path, EMAIL_SCRIPT, external_email)
    recs = run(harness, sid, "email x@gmail.com the numbers")

    assert [r.event.kind for r in recs] == ["user_input", "model_output", "tool_call", "model_output"]
    assert recs[2].action == "blocked"
    # Tool never ran, so no tool_response event and nothing in the outbox.
    assert harness.sandbox(sid).outbox == []
...
def test_monitor_flags_but_executes(tmp_path):
    harness, _, _, _, sid = make(tmp_path, EMAIL_SCRIPT, external_email)
    recs = run(harness, sid, "email x@gmail.com the numbers", mode="monitor")

    call = next(r for r in recs if r.event.kind == "tool_call")
    assert call.action == "flagged"
    assert len(harness.sandbox(sid).outbox) == 1
...
def test_withheld_tool_response_is_replaced(tmp_path):
    script = [
        Resp([Block("tool_use", id="tu_1", name="search_docs", input={"query": "warehouse sync runbook"})], "tool_use"),
        Resp([Block("text", text="Here is the runbook.")], "end_turn"),
    ]
    harness, _, client, _, sid = make(
        tmp_path, script, lambda e: "violation" if e.kind == "tool_response" else "allow"
    )
    recs = run(harness, sid, "find the warehouse sync runbook")
...
def test_classifier_error_fails_closed_in_enforce(tmp_path):
    ...
    harness, _, _, _, sid = make(tmp_path, EMAIL_SCRIPT, boom)
    recs = run(harness, sid, "email someone")
    call = next(r for r in recs if r.event.kind == "tool_call")
...
def test_classifier_sees_prior_events_with_guard_actions(tmp_path):
    harness, _, _, classifier, sid = make(tmp_path, EMAIL_SCRIPT, external_email)
    run(harness, sid, "email x@gmail.com the numbers")
...
def test_policy_loads_rules_and_actions():
    assert {"R1", "R2", "R3", "R3.3", "R4.1", "R6"} <= set(POLICY.rule_ids)
...
def test_off_mode_skips_classifier_and_runs_everything(tmp_path):
    ...
    harness, _, _, classifier, sid = make(tmp_path, EMAIL_SCRIPT, lambda e: "violation")
    usage = TurnUsage()

    async def collect():
        return [r async for r in harness.run_turn(sid, "email x@gmail.com the numbers", "off", usage)]
    ...
    assert len(harness.sandbox(sid).outbox) == 1  # the email really went out (to the sandbox)
...
def test_usage_counts_classifier_calls(tmp_path):
    ...
    async def collect():
        return [r async for r in harness.run_turn(sid, "email x@gmail.com the numbers", "enforce", usage)]
...
def test_export_blind_hides_verdicts_but_keeps_blocks(tmp_path):
    ...
    harness, store, _, _, sid = make(tmp_path, EMAIL_SCRIPT, external_email)
    run(harness, sid, "email x@gmail.com the numbers")
...
def test_policy_parses_rule_titles_and_subrules():
    ...
    assert by_id["R2"].title == "EMAIL"
    assert by_id["R3"].subrules == ("R3.1", "R3.2", "R3.3", "R3.4")
    assert "R4" in POLICY.rule_ids  # a group with no row of its own is still citable


def _policy_copy(tmp_path):
    import shutil

    shutil.copytree(ROOT / "policies" / "ops-agent", tmp_path / "ops-agent")
    return load_policy(tmp_path, "ops-agent")
...
def test_rule_table_renders_into_prompt():
    ...
    assert "- **R5.1** Personal data of the specific person a task is about may be looked up" in text
...
def test_rule_table_add_and_remove(tmp_path):
    ...
    scope = RuleScope(policy, tmp_path / "ops-agent" / "rule_scope.json")
    ...
    assert scope.as_dict()["R8"] == {k: True for k in scope.as_dict()["R1"]}  # new group starts enforced
    saved = json.loads((tmp_path / "ops-agent" / "rules.json").read_text(encoding="utf-8"))
    ...
    policy.rulebook.add({"number": "R2.1", "name": "EMAIL_no_bcc", "prompt": "No BCC."})
...
def test_column_fully_switched_off_skips_classifier(tmp_path):
    ...
    store = Store(tmp_path / "t.db")
    classifier = RuleClassifier(external_email)
    scope = RuleScope(POLICY, tmp_path / "rule_scope.json")
    scope.replace({r.id: {"tool_call": False} for r in POLICY.rules})
    hooks = HookRegistry()
    hooks.register(PolicyGuardHook(POLICY, classifier, scope))
    harness = AgentHarness(FakeClient(EMAIL_SCRIPT), hooks, store, model="claude-opus-5", use_fallbacks=False)
    ...
    assert len(harness.sandbox(sid).outbox) == 1  # so the external email went out
